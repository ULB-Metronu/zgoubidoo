...
def _align_tracks(
    ...
    assert set(particules) == set(
        tracks[identifier].unique(),
    ), f"Required particles not found for element {tracks['LABEL1'].unique()[0]} (are you using Objet5?)."
    ref: _pd.DataFrame = tracks.query(f"{identifier} == '{reference_track}'")[
        coordinates
        + [
            align_on,
            "LABEL1",
    ...
    assert _np.all(_np.diff(ref_alignment_values) >= 0), (
        "The reference alignment values " "are not monotonously increasing"
    )
    ...
    for i, p in enumerate(particules[1:]):
        ...
        for j, c in enumerate(coordinates):
            try:
                assert _np.all(
                    _np.diff(particule[align_on].values) >= 0,
...
def compute_transfer_matrix(beamline: _Input, tracks: _pd.DataFrame) -> _pd.DataFrame:
    ...
    for e in beamline.line:
        ...
        n_dimensions: int = 6
        normalization = [
            2 * (data[i + 1, :, i + n_dimensions] - data[0, :, i + n_dimensions]) for i in range(0, n_dimensions)
        ]
        m = _pd.DataFrame(
            {
