...
def rbend_to_zgoubi(element: _Element, kinematics: _Kinematics, options: Dict) -> List[Command]:
    ...
    m = Multipole(
        LABEL1=element.name[0:_ZGOUBI_LABEL_LENGTH],
        XL=element["L"],
        R0=bore_radius,
...
def sbend_to_zgoubi(element: _Element, kinematics: _Kinematics, options: Dict) -> List[Command]:
    ...
    if options.get("command", Bend) == Bend:
        ...
        if element.K1 is not None and not _np.isnan(element.K1):
            field_index = (
                _np.abs(
                    element.L / element.ANGLE,
                ).to("m")
                ** 2
                * getattr(element, "K1", 0.0 * _ureg.m**-2)
            ).magnitude
        ...
        if element.K2 is not None and not _np.isnan(element.K1):
            field_index_B = (
                _np.abs(
                    element.L / 2 * element.ANGLE,
                ).to("m")
                ** 3
                * getattr(element, "K2", 0.0 * _ureg.m**-3)
            ).magnitude
...
def quadrupole_to_zgoubi(element: _Element, kinematics: _Kinematics, options: Dict) -> List[Command]:
    ...
    if (
        element.get("B1") is not None
        and element.get("R") is not None
        and not _np.isnan(element["B1"])
        ...
        bore_radius = options.get("R0", 10 * _ureg.cm)
        if (
            element.get("K1") is None
            and element.get("K1L") is None
            and element.get("K1BRHO") is None
            and element.get(
                "K1S",
            )
            is None
            and element.get("K1SL") is None
        ):
...
def solenoid_to_zgoubi(element: _Element, kinematics: _Kinematics, options: Dict) -> List[Command]:
    ...
    if (
        element.get("B0") is not None
        and element.get("R") is not None
        and not _np.isnan(element["B0"])
        and not _np.isnan(element["R"])
    ):
        b_field = element["B0"]
        bore_radius = element["R"]
...
def sextupole_to_zgoubi(element: _Element, kinematics: _Kinematics, options: Dict) -> List[Command]:
    ...
    if (
        ...
        bore_radius = options.get("R0", 10 * _ureg.cm)
        if (
            element.get("K2") is None
            and element.get("K2L") is None
            and element.get("K2BRHO") is None
            and element.get(
                "K2S",
            )
            is None
            and element.get("K2SL") is None
        ):
            gradient = 0 / _ureg.m**2
            ...
            if element.get("K2L") == 0:
                ...
                changeref_in = ChangRef("changeref_in", TRANSFORMATIONS=[("XR", -45 * _ureg.degree)])  # noqa: F841
                changeref_out = ChangRef("changeref_out", TRANSFORMATIONS=[("XR", 45 * _ureg.degree)])  # noqa: F841
...
def octupole_to_zgoubi(element: _Element, kinematics: _Kinematics, options: Dict) -> List[Command]:
    ...
    if (
        ...
        bore_radius = options.get("R0", 10 * _ureg.cm)
        if (
            element.get("K3") is None
            and element.get("K3L") is None
            and element.get("K3BRHO") is None
            and element.get(
                "K3S",
            )
            is None
            and element.get("K3SL") is None
        ):
            gradient = 0 / _ureg.m**2
            ...
            changeref_in = ChangRef("changeref_in", TRANSFORMATIONS=[("XR", -45 * _ureg.degree)])  # noqa F8441
            changeref_out = ChangRef("changeref_out", TRANSFORMATIONS=[("XR", 45 * _ureg.degree)])  # noqa F8441
        else:
