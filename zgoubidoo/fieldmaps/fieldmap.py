...
can then be easily used in a line. Fitting methods are also available to calculate the field decay based on an Enge
function.
"""
import os
import shutil
...
def load(file: str, path: str = ".") -> _pd.DataFrame:
    ...
    if os.path.basename(file).startswith("b_"):  # This is a binary file
        fieldmap = _pd.DataFrame(
            data=_np.fromfile(os.path.join(path, file)).reshape(-1, 6),
            columns=["Y", "Z", "X", "BY", "BZ", "BX"],
        )
    else:
        fieldmap = _pd.read_csv(
...
class FieldMap:
    ...
    def load_from_opera(cls, file: str, path: str = "."):
        """
        Factory method to load a field map from an Opera parent file.
    ...
    def generate_from_expression(
        cls,
        bx_expression: _sp = None,
        by_expression: _sp = None,
        bz_expression: _sp = None,
        mesh: _np.ndarray = None,
        use_njit: bool = True,
        generate_3d_map: bool = False,
        nterms: int = 4,
    ):
        """
        ...
        """
        if generate_3d_map:
            bx_expression, by_expression, bz_expression = cls.get_off_plane_field(
                bx_expression=bx_expression or 0,
        ...
        return cls(
            field_map=generate_from_expression(
    ...
    def get_off_plane_field(
        ...
        b_x = (
            bx_off.replace(bx_mid, bx_expression)
            .replace(by_mid, by_expression)
            .replace(
                bz_mid,
                bz_expression,
            )
        ...
        b_y = (
            by_off.replace(bx_mid, bx_expression)
            .replace(by_mid, by_expression)
            .replace(
                bz_mid,
                bz_expression,
        ...
        b_z = (
            bz_off.replace(bx_mid, bx_expression)
            .replace(by_mid, by_expression)
            .replace(
                bz_mid,
                bz_expression,
    ...
    def plot_field_profile(self, ax, field_component: str = "MOD", sampling_method: str = "nearest"):
        ...
        if isinstance(ax, _ZgoubidooMatplotlibArtist):
            ...
            raise _ZgoubidooException(
                f"{ax} is not a valid artist. Use ZgoubidooPlotlyArtist() or " f"ZgoubidooMatplotlibArtist",
            )
    ...
    def plot_field_map(
        ...
        if isinstance(ax, _ZgoubidooMatplotlibArtist):
            ...
            if self.reference_trajectory is not None:
                ax.scatter(
                    x=self.reference_trajectory[:, plane1],
...
class CartesianFieldMap(FieldMap):
    def __call__(
        ...
        **kwargs,
        ...
        if generator is None:
            generator = _fieldmaps.ToscaCartesian3D
    ...
    @classmethod
    def generate_from_cartesian_expression(
        cls,
        bx_expression: _sp = None,
        by_expression: _sp = None,
        bz_expression: _sp = None,
    ...
    def attach_cartesian_trajectory(
        self,
        axis: str = "X",
...
class PolarFieldMap(FieldMap):
    def __init__(
        self,
        field_map: _pd.DataFrame,
        mesh_sampling_radius: _np.ndarray = 0,
        mesh_sampling_theta: _np.ndarray = 0,
        mesh_sampling_z: _np.ndarray = 0,
    ):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if generator is None:
            generator = _fieldmaps.ToscaPolar
    ...
    @classmethod
    def generate_from_polar_expression(
        cls,
        bx_expression: _sp = None,
        by_expression: _sp = None,
        bz_expression: _sp = None,
        radius: _np.ndarray = None,
        theta: _np.ndarray = None,
        vertical_positions: _np.ndarray = None,
    ):
        """
        Factory method to generate a field map from analytic expressions
        ...
        return cls(
            ...
            mesh_sampling_radius=radius,
            mesh_sampling_theta=theta,
            mesh_sampling_z=vertical_positions,
        )

    def attach_polar_trajectory(
        self,
        radius: float,
        lower_angle: float,
        upper_angle: float,
